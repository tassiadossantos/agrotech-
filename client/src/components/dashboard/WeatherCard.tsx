      ...
      <CardContent>
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-4xl font-bold font-outfit text-foreground">{weather.temp}°C</span>
              <p className="text-xs text-muted-foreground font-medium mt-1">{selectedFarm.name}</p>
            </div>
          ...
          <div className="grid grid-cols-3 gap-2 mt-2">
            ...
            <div className="flex flex-col items-center p-2 bg-background/50 rounded-lg border border-border/50">
              <span className="text-lg leading-none">
                {weather.forecast === 'rainy' ? '🌧️' : weather.forecast === 'sunny' ? '☀️' : '☁️'}
              </span>
